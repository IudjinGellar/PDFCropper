...
def crop(widgets):
    ...
    try:
        ...
        widgets['message_lbl'].config(
            text='Страницы должны быть числами!')
        return
    try:
        assert page_to - page_from >= 0, Exception
    except AssertionError:
        widgets['message_lbl'].config(
            text='Некорректно введены страницы')
        return
    check_new_filename = new_file_name.isalnum()
    if not check_new_filename:
        widgets['message_lbl'].config(
            text='Имя файла только из букв и цифр.')
        return
    check_save_path = exists(save_path)
    if not check_save_path:
        widgets['message_lbl'].config(
            text='Несуществующая директория для сохранения')
        return
...
def clear_fields(widgets):
    for widget in widgets.values():
        if widget in (widgets['root'], widgets['message_lbl']):
            continue
        if widget.get():
...
def create_gui():
    ...
    file_path_choose = tk.Button(
        filename_frame, text='Select file',
        command=(lambda: choose_file(widgets)))
    ...
    tk.Button(new_file_path_frame, text='Сохранить в',
              command=(lambda: choose_dir(widgets))).pack(side=tk.RIGHT)
    new_file_path_frame.pack(side=tk.TOP)

    # кнопки действий
    btns_frame = tk.Frame(root)
    tk.Button(btns_frame, text='Обрезать',
              command=(lambda: crop(widgets))).pack(side=tk.RIGHT)
    tk.Button(btns_frame, text='Отчистить',
              command=(lambda: clear_fields(widgets))).pack(side=tk.RIGHT)
